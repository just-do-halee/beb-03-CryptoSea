...
const Details = (props) => {
    ...
    const { loading, data: { getNFTs: { ok = [] } = {} } = {} } = useQuery(...QUERY_GetNFT(tid));

    api.ownerOf(tid).then(setOwner);

    const { name, description, url, transaction: { txhash } = {}, attributes } = ok[0] || {};

    const price = 0;
    if (txhash) {
        api.eth.getTransactionReceipt('0x' + txhash).then(({ from = LOADING } = {}) => setCreator(from));
    }
                ...
                <Detail>
                    ...
                    <Accordion expanded={expanded}>
                        <div className='details'>
                            <AccordionSummary expandIcon={<ExpandMoreIcon />}
                                aria-controls="panel1a-content" id="panel1a-header" onClick={() => {
                                    setExpanded(!expanded);
                                }}>
